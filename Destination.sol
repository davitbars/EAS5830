...
contract Destination is AccessControl {
	...
	{
		address wrappedAddress = underlying_tokens[_underlying_token];
		require(wrappedAddress != address(0), "Token not registered");

		BridgeToken(wrappedAddress).mint(_recipient, _amount);

		emit Wrap(_underlying_token, wrappedAddress, _recipient, _amount);
	}
