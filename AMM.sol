...
contract AMM is AccessControl {
	...
	*/
	function tradeTokens(address sellToken, uint256 sellAmount) public {
		...
		uint256 reserveSell = sell.balanceOf(address(this));
		uint256 reserveBuy = buy.balanceOf(address(this));

		uint256 sellAmountAfterFee = (sellAmount * (10000 - feebps)) / 10000;

		require(sell.transferFrom(msg.sender, address(this), sellAmount), "Transfer failed");

		uint256 newReserveSell = reserveSell + sellAmountAfterFee;
		uint256 newReserveBuy = invariant / newReserveSell;
		uint256 buyAmount = reserveBuy - newReserveBuy;

		require(buyAmount > 0, "Output too small");
		require(buy.transfer(msg.sender, buyAmount), "Transfer out failed");
	...
	*/
	function provideLiquidity(uint256 amtA, uint256 amtB) public {
		...
		//YOUR CODE HERE
		if (amtA > 0) {
			require(IERC20(tokenA).transferFrom(msg.sender, address(this), amtA), "Transfer tokenA failed");
		}
		if (amtB > 0) {
			require(IERC20(tokenB).transferFrom(msg.sender, address(this), amtB), "Transfer tokenB failed");
		}
